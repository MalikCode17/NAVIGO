...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(btnPay) {
        btnPay.addEventListener('click', function() {
            ...
            setTimeout(() => {
                let bookings = JSON.parse(localStorage.getItem('navigo_bookings')) || [];
                const pendingIndex = bookings.findLastIndex(b => b.status === 'pending' && b.type === 'car');
                if (pendingIndex !== -1) {
                ...
                if (window.addNotification) {
                    window.addNotification("Pembayaran Rental Mobil Berhasil", "Voucher rental mobil Anda telah diterbitkan.", "success");
                }

                window.location.href = 'voucher-car.html';
            }, 2000);
        });
    }
    ...
    if (btnCancel) {
        btnCancel.addEventListener('click', function(e) {
            e.preventDefault();
            if (confirm("Yakin ingin membatalkan pesanan rental mobil ini?")) {
                let bookings = JSON.parse(localStorage.getItem('navigo_bookings')) || [];
                const pendingIndex = bookings.findLastIndex(b => b.status === 'pending' && b.type === 'car');
                ...
                if (window.addNotification) {
                    window.addNotification("Pesanan Mobil Dibatalkan", "Pesanan rental mobil telah dibatalkan.", "cancel");
                }
