...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(btnPay) {
        btnPay.addEventListener('click', function() {
            ...
            setTimeout(() => {
                // Update order status
                let bookings = JSON.parse(localStorage.getItem('navigo_bookings')) || [];
                // Temukan booking hotel yang masih pending dari orang ini (yang terakhir)
                const pendingIndex = bookings.findLastIndex(b => b.status === 'pending' && b.type === 'hotel');
                if (pendingIndex !== -1) {
                ...
                if (window.addNotification) {
                    window.addNotification("Pembayaran Hotel Berhasil", "Pesanan hotel Anda telah dikonfirmasi.", "success");
                }

                window.location.href = 'voucher-hotel.html';
            }, 2000);
        });
    }
    ...
    if (btnCancel) {
        btnCancel.addEventListener('click', function(e) {
            e.preventDefault();
            if (confirm("Yakin ingin membatalkan pesanan hotel ini?")) {
                let bookings = JSON.parse(localStorage.getItem('navigo_bookings')) || [];
                const pendingIndex = bookings.findLastIndex(b => b.status === 'pending' && b.type === 'hotel');
                ...
                if (window.addNotification) {
                    window.addNotification("Pesanan Hotel Dibatalkan", "Pesanan hotel telah dibatalkan.", "cancel");
                }
